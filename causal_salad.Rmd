...
# cmdstanr::install_cmdstan(overwrite=TRUE, cores=4)
```

# TWO MOMS example

## 1. Example without confound
...
### Generate models without confound (include models not shown in video)
```{r}
plot( coeftab( lm( D ~ M ) , 
               lm( D ~ M + B1 ) ,  
               lm( D ~ M + B2 ) ,
...
## 2. Example with confound
### Simulate variables with confound
```{r}
set.seed(1908)
N <- 200 # number of pairs

# simulate confounds (U)
U <- rnorm(N) 

# birth order of mum
B1 <- rbinom(N,size=1,prob=0.5)
# family order of mum
M <- rnorm( N , 2*B1 + U ) #### Add confound

# birth order of daughter
B2 <- rbinom(N,size=1,prob=0.5)
# family order of daughter
D <- rnorm( N , 2*B2 + U + 0*M ) #### Add confound
```

### Generate model with confound, it gets worse for B1
```{r}
...
### Generate models with confound (not shown in video)
```{r}
plot( coeftab( lm( D ~ M ) , 
               lm( D ~ M + B1 ) ,  
               lm( D ~ M + B2 ) ,
...
## 3. Other approaches
### Note the best case scenario
```{r}
# best case scenario, if we observed confound
precis( lm( D ~ M + B2 + U ) )
## precis function displays concise parameter estimate information for an existing model fit
```

### Compare above with precis output from bayesian inference
...
### Plot estimated and simulated confound
```{r}
...
## Simulation of no discrimination
```{r eval=FALSE}
